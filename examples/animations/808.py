...
# Also you can preview audio directly within the Coldtype viewing app, if you provide a file (readable by python’s `wave` package) to the the @animation renderable. Here we define the path as a variable for referencing below in the `@animation`.
...
@animation(duration=drums.duration)
def drummachine(f):

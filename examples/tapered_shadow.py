...
def outline(f):
    ...
    .layer(
        lambda p: p.f(hsl(0.3)).s(hsl(0.3, 1, 0.75)).sw(30),
        lambda p: (p.pen().layer(
            lambda p: p.f(1).s(1).sw(100)
                .cond(SHAKE, warp(-1, rs[f.i//4], mult=10)),
            lambda p: p.f(0).s(0).sw(17))
            .ch(phototype(f.a.r, blur=3, cut=200, cutw=20, fill=hsl(0.75, 1, 0.605)))),
        lambda p: p.pen().f(1)
            .ch(phototype(f.a.r,
                blur=3, cutw=15,
...
@animation(bg=0, timeline=Timeline(92, 23.976), solo=1)
def taper(f):
    return (StSt("COLD\nTYPE", fnt, 330,

...
def test_system_font(r):
    return RunonPen().text("Hello, world!", Style("Times", 100, load_font=0, fill=0), r.offset(100, 100))
...
def test_return_string(r, rs):
    ...
    return DATPens([
        (StyledString("COLDTYPE", style)
            ...
            .translate(-5, 17)
            .f(hsl(0.9))),
        (RunonPen().text("COLDTYPE", style, rt)
            .translate(0, 0))])
...
def test_midi(r, rs):
    ...
    return RunonPen().text("CLDTYP", style, r.inset(50))
...
def render(f):
    ...
    dps = Graf(T2L("COLD\nTYPE", s), f.a.r.inset(90, 0), leading=20).fit().pens().interleave(lambda p: p.outline(outline).f(0) if p.glyphName else p).align(f.a.r).scale(0.85, center=False).align(f.a.r)
